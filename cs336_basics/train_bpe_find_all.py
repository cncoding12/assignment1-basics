...
def train_bpe(
    ...
    """
    高效的 Byte-Level BPE 分词器训练函数（使用倒排索引进行增量更新）。
    """
    # 1. 初始化基础词表 (0-255 字节)
    vocab: dict[int, bytes] = {i: bytes([i]) for i in range(256)}

    # 2. 将 special_tokens 加入词表
    ...
    # 4. 根据 special_tokens 切分文本
    if special_tokens:
        special_regex = "|".join(
            re.escape(tok) for tok in sorted(special_tokens, key=len, reverse=True)
        )
        chunks = [c for c in re.split(special_regex, text) if c]
    else:
        chunks = [text]

    # 5. GPT-2 风格正则表达式预分词（使用 findall 替代 finditer 提升速度）
    gpt2_pat = re.compile(
    ...
    for _ in range(num_merges):
        ...
        # 选出最高频 pair；频次相同时，按字节字典序大的优先 (tie-break)
        ...
        # 获取包含 best_pair 的词（仅更新这些词，避免全量扫描）
